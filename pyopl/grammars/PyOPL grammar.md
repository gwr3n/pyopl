...
- Statement termination and whitespace:
  ...
  - Therefore, layout around `forall` + labels is whitespace-insensitive.

Allowed forall + label layouts (equivalent and valid):
```opl
// single-statement form
forall(h in H) Dominance: sum(i in I) v[i]*x[h][i] - sum(r in R) u[r]*y[h][r] <= 0;

// block form
forall(h in H) {
  Dominance: sum(i in I) v[i]*x[h][i] - sum(r in R) u[r]*y[h][r] <= 0;
}

// multiline single-statement form
forall(h in H)
  Dominance:
    sum(i in I) v[i]*x[h][i] - sum(r in R) u[r]*y[h][r] <= 0;
```

- Reusing the same unindexed label across constraints generated by a forall is permitted. The parser records a label_template with iterator names; code generators may expand it to unique names (e.g., `lbl[i,j]` or `lbl[i=...,j=...]`).
