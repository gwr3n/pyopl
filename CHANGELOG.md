...
- Compare inline parameter definitions in indexed equivalence proofs and return an inconclusive result when index safety cannot interpret a declared domain.
- Validate indexed accesses inside `forall` constraint bodies during index-safety preflight.
- Narrow finite integer iterator ranges when a conditional branch excludes a range endpoint.
- Display unknown model-comparison outcomes as undetermined instead of non-equivalent in the IDE.
...
- Replaced oversized GenAI and exemplar error-dialog tracebacks with concise, actionable messages while retaining detailed diagnostics in the output.
- Aligned the human-readable and machine-oriented PyOPL grammars with indexed tuple-set arrays, filtered integer-set comprehensions, and tuple-set values nested in `.dat` arrays.

### Changed

- Expanded the PyOPL grammar documentation with the implemented BNF, declaration and expression rules, semantic clarifications, backend limitations, and model/data examples.
