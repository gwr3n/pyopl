...
from pyopl.model_equivalence import AbstractEquivalenceResult, EquivalenceResult
...
class TestBatchCompare(unittest.TestCase):
    def test_compares_matching_data_basenames_and_writes_reports(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            result = EquivalenceResult(
                ...
                proof_steps=("matched",),
                counterexample=None,
                scope="supplied_instances",
                variable_mapping=(("left_x", "right_x"),),
            )
            ...
            self.assertEqual(payload["instances"][0]["scope"], "supplied_instances")
            self.assertEqual(payload["instances"][0]["variable_mapping"], {"left_x": "right_x"})
            markdown = (root / "left_vs_right.md").read_text(encoding="utf-8")
            self.assertIn("| a.dat | equivalent | True | normalized |", markdown)
    ...
    def test_records_comparison_errors_and_continues(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(report["instances"][0]["reason"], "invalid")
            self.assertFalse(report["instances"][1]["equivalent"])
