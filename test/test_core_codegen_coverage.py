...
class TestCodeGeneratorCoverage(unittest.TestCase):
    ...
    def test_generated_solvers_apply_injected_settings(self):
        ...
        self.assertIn("elif model.status == GRB.TIME_LIMIT:", gurobi_code)
        self.assertIn("results['status'] = 'TIME_LIMIT'", gurobi_code)
        self.assertIn("results['message'] = 'Time limit reached.'", gurobi_code)
        self.assertIn("if model.SolCount > 0:", gurobi_code)
        self.assertIn("print('Incumbent solution found:')", gurobi_code)
        self.assertLess(gurobi_code.index("results['stats'] = stats"), gurobi_code.index("if model.status == GRB.OPTIMAL:"))
        self.assertIn("stats = {'Status': model.status}", gurobi_code)
        self.assertIn("except (AttributeError, gp.GurobiError):", gurobi_code)
        self.assertIn("stats['Runtime'] = model.Runtime", gurobi_code)
        self.assertIn("stats['NodeCount'] = model.NodeCount", gurobi_code)
        self.assertIn("stats['IterCount'] = model.IterCount", gurobi_code)
        ...
        self.assertIn("has_incumbent = res.x is not None and res.fun is not None", scipy_code)
        self.assertIn("status_str == 'TIME_LIMIT' and has_incumbent", scipy_code)
        self.assertIn("print('Incumbent solution found:')", scipy_code)
